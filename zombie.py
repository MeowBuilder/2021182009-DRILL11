...
class Zombie:
    ...
    def draw(self):
        if self.dir < 0:
            Zombie.images['Walk'][int(self.frame)].composite_draw(0, 'h', self.x, self.y + (50 * (self.HP - 2)), self.HP * 100, self.HP * 100)
        else:
            Zombie.images['Walk'][int(self.frame)].draw(self.x, self.y + (50 * (self.HP - 2)), self.HP * 100, self.HP * 100)
